...
One dataset, one master: the standard is mastered in this repository, the register in `06-ecosystem/registry/`, and each product in its own repository. Everything else that looks like a copy is a mirror or a rendered artifact (for example, the ver.cy site).
...
The operational side, the full process palette with roles, delegation tiers (human-in-the-loop, human-on-the-loop, autonomous-with-audit), execution variants and the mandatory minimum, is the responsibility of the [processes repository](https://github.com/ver-cy/processes): 81 processes in 7 families, with BPMN 2.0 diagrams and worked bootstrap examples.
